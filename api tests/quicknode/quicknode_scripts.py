...
def explore_json_structure(data, prefix="", visited=None):
    """
    Recursively explore and print all fields in a JSON structure
    while handling circular references
    """
    if visited is None:
        visited = set()
    ...
    if isinstance(data, dict):
        for key, value in data.items():
            ...
            if id(value) not in visited:
                ...
                explore_json_structure(value, full_path, visited)
                
    elif isinstance(data, list):
        print(f"{prefix} (List with {len(data)} items)")
        if data:  # If list is not empty, show structure of first item
            explore_json_structure(data[0], f"{prefix}[0]", visited)
            
    elif isinstance(data, (str, int, float, bool)) or data is None:
        print(f"{prefix}: {data}")

def analyze_transaction_structure(tx):
    ...
    try:
        ...
        print("=====================")
        explore_json_structure(tx_json)
